...
router.post("/AllPlagiarismDefense", authenticateToken, async (req, res) => {
...
router.get("/plagiarism-defense/:id/plagiarism-file", authenticateToken, async (req, res) => {
	await sendPdfByType(req, res, "plagiarism_file");
});
router.get("/plagiarism-defense/:id/full-report-file", authenticateToken, async (req, res) => {
...
async function sendPdfByType(req, res, field) {
	try {
		...
		const pool = await poolPromise;
		const q = await pool.request().input("id", id).query(`SELECT ${field} FROM plagiarism_defense WHERE plagiarism_report_id = @id`);
		...
		try {
			...
			const result = await pool
				...
				.input("full_report_file", fullReportFilePath).query(`INSERT INTO plagiarism_defense (
					...
					@plagiarism_file,
					@full_report_file)`);
			await pool.request().input("research_name", research_name).input("request_thesis_defense_id", request_thesis_defense_id).query(`
			...
			console.error("addPlagiarismDefense:", err);
			res.status(500).json({ message: "เกิดข้อผิดพลาดในการบันทึกคำร้องขอสอบ" });
		}
	}
);

router.post("/approvePlagiarismDefense", authenticateToken, async (req, res) => {
