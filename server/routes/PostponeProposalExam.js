...
router.post("/allPostponeProposalExam", authenticateToken, async (req, res) => {
	...
	try {
		...
		let query = `
			SELECT rqc.*,
      		 	...
      		 	rq.major_id,
       			rq.faculty_name
			FROM postpone_proposal_exam rqc
			LEFT JOIN request_thesis_proposal rq
				ON rqc.request_thesis_proposal_id = rq.request_thesis_proposal_id
			`;
		if (role === "student") {
			if (lastRequest) {
				query = `
		...
		query += " ORDER BY rqc.postpone_proposal_exam_id DESC";
		...
		console.error("allPostponeProposalExam:", err);
		res.status(500).json({ message: "เกิดข้อผิดพลาดในการดึงข้อมูลคำร้อง" });
	}
});

router.post("/addPostponeProposalExam", authenticateToken, async (req, res) => {
	const { request_thesis_proposal_id, reason, request_type, thesis_exam_date } = req.body;
	const pool = await poolPromise;
	...
	try {
		...
		const result = await request
			...
			.input("status", "7").query(`
				INSERT INTO postpone_proposal_exam (
			...
			`);
		await request.query(`UPDATE request_thesis_proposal SET status = @status WHERE request_thesis_proposal_id = @request_thesis_proposal_id`);
...
router.post("/approvePostponeProposalExam", authenticateToken, async (req, res) => {
	const { postpone_proposal_exam_id, request_thesis_proposal_id, name, selected, comment, thesis_exam_date } = req.body;
	const { role } = req.user;
	...
	try {
		const request = new sql.Request(transaction);
		const result = await request
			.input("postpone_proposal_exam_id", postpone_proposal_exam_id)
			.input("statusCancel", statusCancel)
			.input("name", name)
			.input("approve", selected === "approve" ? 1 : 0)
			.input("date", formatDateForDB())
			.input("comment", comment).query(`
				UPDATE postpone_proposal_exam
				SET ${roleFields[role]},
					status = @statusCancel
					${selected === "approve" ? "" : ", comment = @comment"}	
				OUTPUT INSERTED.* WHERE postpone_proposal_exam_id = @postpone_proposal_exam_id
			`);
		if (role === "chairpersons" && selected === "approve") {
			await request.input("request_thesis_proposal_id", request_thesis_proposal_id).input("thesis_exam_date", thesis_exam_date).input("status", status).query(`
			UPDATE request_thesis_proposal
			SET status = @status, thesis_exam_date = @thesis_exam_date
			OUTPUT INSERTED.* WHERE request_thesis_proposal_id = @request_thesis_proposal_id
		`);
		} else {
			await request.input("request_thesis_proposal_id", request_thesis_proposal_id).input("status", status).query(`
			UPDATE request_thesis_proposal
			SET status = @status
			OUTPUT INSERTED.* WHERE request_thesis_proposal_id = @request_thesis_proposal_id
		`);
