...
const PrintExam = () => {
	...
	const [selectedType, setSelectedType] = useState("ขอสอบประมวลความรู้");
	...
	const filteredData = useMemo(() => {
		return request.filter((p) => {
			...
			return filterStatus && matchesSearch && matchesType && matchesLevel;
		});
	}, [request, search, selectedType, filterLevel]);

	const handleExport = () => {
		const mainHeader = [[`รายชื่อผู้มีสิทธิสอบ${selectedType} ปีการศึกษา ${selectedTerm}`]];
		...
		const fileName = `รายชื่อผู้มีสิทธิสอบ${selectedType}_${selectedTerm}.xlsx`;
		...
		<Box>
			...
			<Group justify="space-between">
				<Group>
					...
					<Select placeholder="ชนิดคำขอ" data={["ขอสอบประมวลความรู้", "ขอสอบวัดคุณสมบัติ"]} value={selectedType} allowDeselect={false} onChange={setSelectedType} />
