//ผลการสอบโครงร่างวิทยานิพนธ์/การค้นคว้าอิสระ
...
const ExamResults = () => {
	...
	useEffect(() => {
		const fetchData = async () => {
			try {
				const res = await fetch(`${BASE_URL}/api/AllExamProposalResults`, {
					method: "POST",
					headers: { "Content-Type": "application/json", Authorization: `Bearer ${token}` },
	...
	const handleSaveExamResults = async () => {
		try {
			const req = await fetch(`${BASE_URL}/api/AddExamProposalResults`, {
				method: "POST",
				headers: { "Content-Type": "application/json", Authorization: `Bearer ${token}` },
	...
	return (
		<Box>
			...
			<Text size="1.5rem" fw={900} mb="md">
				{role === "officer_major" ? "กรอก" : "พิมพ์"}ผลการสอบโครงร่างวิทยานิพนธ์/การค้นคว้าอิสระ
			</Text>
			{!selectedGroupId ? (
				<Box>
					<Group>
