...
const RequestExam = () => {
	...
	const handleOpenAdd = async () => {
		try {
			const req = await fetch(`${BASE_URL}/api/student/${user_id}`, {
				method: "GET",
	...
	const handlePay = async (item) => {
		try {
			const requestRes = await fetch(`${BASE_URL}/api/payRequestExam`, {
				method: "POST",
				headers: { "Content-Type": "application/json", Authorization: `Bearer ${token}` },
				body: JSON.stringify({ request_exam_id: item.request_exam_id, receipt_vol: "2569/RRT005", receipt_No: "1", receipt_pay: education_level === "ปริญญาโท" ? 1000 : 1500 }),
			});
			const requestData = await requestRes.json();
			if (!requestRes.ok) {
