...
const ExamResults = () => {
	...
	useEffect(() => {
		const fetchData = async () => {
			try {
				const res = await fetch("http://localhost:8080/api/AllExamProposalResults", {
					method: "POST",
					headers: { "Content-Type": "application/json", Authorization: `Bearer ${token}` },
				});
				const data = await res.json();
	...
	const handleSaveExamResults = async () => {
		try {
			const req = await fetch("http://localhost:8080/api/AddExamProposalResults", {
				method: "POST",
				headers: { "Content-Type": "application/json", Authorization: `Bearer ${token}` },
	...
	return (
		<Box>
			...
			<Text size="1.5rem" fw={900} mb="md">
				กรอกผลการสอบโครงร่างวิทยานิพนธ์/การค้นคว้าอิสระ
			</Text>
					...
					<ScrollArea type="scroll" offsetScrollbars style={{ borderRadius: 8, border: "1px solid #e0e0e0" }}>
						<Table highlightOnHover>
							...
							<Table.Tbody>
								{Object.entries(
									group
										.filter((i) => !selectedTerm || i.term === selectedTerm)
